...
class DDPMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        alphas_prod = self.alphas_cumprod[t]
        sqrt_alpha = torch.sqrt(alpha)
        sqrt_one_minus_alphas_prod = torch.sqrt(1-alphas_prod)
        
        mu = 1/sqrt_alpha * (x_t - one_minus_alpha / sqrt_one_minus_alphas_prod * eps_theta)
        z_t = torch.randn_like(x_t)
        sample_prev = mu + self.sigmas[t] * z_t
        #######################
    ...
    def add_noise(
        ...
        alphas_prod = self.alphas_cumprod[t]
        sqrt_alphas_prod = torch.sqrt(alphas_prod).view(-1, 1, 1, 1)
        sqrt_one_minus_alphas_prod = torch.sqrt(1-alphas_prod).view(-1, 1, 1, 1)
        
        x_t = sqrt_alphas_prod*x_0 + sqrt_one_minus_alphas_prod*eps
        #######################
